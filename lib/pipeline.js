 ...
 */
import { recordVerification, recordWorkstream, readState, writeState } from './worktree.js';
 ...
 *
 * If an investigation workstream reports changed files, the report is
 * rejected — that is implementation work wearing the wrong label.
 */
export function validateWorkstream(state, { kind, files = [] }) {
  if (kind === 'investigation' && files.length > 0) {
    return {
      ok: false,
...
export function report(workspace, runId, reportEntry) {
  const state = readState(workspace, runId);
  if (!state) return { ok: false, reason: `unknown run ${runId}` };
  const check = validateWorkstream(state, reportEntry);
